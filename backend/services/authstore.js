...
/* --------------------------------------------------------------- cache */
// A tiny in-memory cache with a short TTL. Two jobs:
//   1. requireAuth now re-validates the user on EVERY request (so disabling or
//      deleting a user takes effect live). Without a cache that would hit
//      GitHub on every poll — the cache bounds it to ~once per TTL per instance.
//   2. Read-after-write: a create/deactivate reflects instantly for this
//      instance instead of waiting on GitHub's read-after-write lag.
...
async function loadDepartments() {
  ...
  if (f && Array.isArray(f.content?.items) && f.content.items.length) {
    _depts = { data: f.content, at: now };
    return _depts.data;
  }
  // Read failed or empty. Never overwrite real data we already hold in cache.
  if (_depts.data && _depts.data.items?.length) return _depts.data;
...
async function saveDepartments(content) {
  ...
  _depts = { data: content, at: Date.now() }; // reflect immediately after a confirmed write
}
...
async function saveUsers(content) {
  ...
  _users = { data: content, at: Date.now() }; // reflect immediately after a confirmed write
}
